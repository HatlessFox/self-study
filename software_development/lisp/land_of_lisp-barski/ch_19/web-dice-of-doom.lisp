(load "../ch_18/dice-of-doom-v2")
(load "../ch_17/svg_draw")
(load "../ch_13/http-server")

;; TODO: deal with 5x5 performance
...
(defun draw-die-svg (x y col)
  (labels ((calc-pt (pt) (cons (+ x (* *dice-scale* (car pt)))
          ...
          (mapc (lambda (x y)
                ...
                '(-0.875 -0.80 -0.725 -0.775 -0.7 -0.625 -0.35 -0.05 -0.45 -0.15
                  -0.45 -0.05))))
  ...
  (loop for y below *board-size*
        do (loop for x below *board-size*
                 ...
                 for col = (brightness (nth (first hex) *die-colors*)
                                       (* -15 (- *board-size* y)))
                 do (if (or (member pos legal-tiles) (eq pos chosen-tile))
                        (tag g ()
                             (tag a ("xlink:href" (make-game-link pos))
                                  (draw-tile-svg x y pos hex xx yy col
                                                 chosen-tile)))
                      (draw-tile-svg x y pos hex xx yy col chosen-tile)))))

(defun make-game-link (pos) (format nil "/game.html?chosen=~a" pos))

;; (with-open-file (*standard-output* "test.svg"
;;                  :direction :output
;;                  :if-exists :supersede)
...
(defun dod-request-handler (path header params)
  (http-respond-ok 
   (if (not (equal path "game.html")) "Sorry... I don't know that page."
       ...
       (tag center ()
            ...
            (let ((chosen (assoc 'chosen params)))
              (when (or (not *cur-game-tree*) (not chosen))
                (setf chosen nil)
                (web-initialize))
              (cond ((lazy-null (gt-moves *cur-game-tree*))
                     (web-announce-winner (gt-board *cur-game-tree*)))
                    ((zerop (gt-player *cur-game-tree*))
                     (web-handle-human
...
(defun web-handle-human (pos)
  (cond ((not pos)
        ...
        ((eq pos 'pass)
         (setf *cur-game-tree* (cadr (lazy-car (gt-moves *cur-game-tree*))))
...
(defun draw-dod-page (tree selected-tile)
  (svg (width *board-width* height *board-height*)
       (draw-board-svg (gt-board tree)
                       selected-tile
                       (take-all
                        (if selected-tile
                            (lazy-mapcar (lambda (m)
                                           (when (eql (caar m) selected-tile)
                                             (cadar m)))
                                         (gt-moves tree))
                          (lazy-mapcar #'caar (gt-moves tree)))))))

;;(serve #'dod-request-handler)
;; [goto] http://localhost:8080/game.html
