                        ...
                        (make-lazy (list
                          (list
                           ...
                           (game-tree
                            ...
                            cur-player (+ spare-dice (dice dst)) nil)
                           (game-tree
...
(defun board-attack-fail (board player src dst dice)
  (board-array (loop for pos from 0
                     ...
                     collect (if (eq pos src) (list player 1) hex))))
...
(defun roll-against (src-dice dst-dice)
  (< (roll-dice dst-dice) (roll-dice src-dice)))

(defun pick-chance-branch (board move)
  (labels ((dice (pos) (cadr (aref board pos))))
          (let ((path (car move)))
            (if (or (null path)
                    (roll-against (dice (car path)) (dice (cadr path))))
                (cadr move) (caddr move)))))
