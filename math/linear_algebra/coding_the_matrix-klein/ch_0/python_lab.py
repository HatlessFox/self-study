...
data_19 = ['A', 'B', 'C', 'D', 'E']                         
range_and_zip = list(zip(range(len(data_19)), data_19))
...
representation_dict =  { ((i * base) + j) * base + k:[i, j, k] for i in digits for j in digits for k in digits}
...
def all_3_digit_numbers(base, digits): return {((i*base)+j)*base+k for i in digits for j in digits for k in digits}
