...
def create_voting_dict(strlist, party=None):
    """
    ...
    for rec in strlist:
        es = rec.split(' ')
        if party and es[1] != party:
            continue;
        records[es[0]] = [ int(vote_rec) for vote_rec in es[4:]] # for submit
        # records[es[0]] = [ int(vote_rec) for vote_rec in es[3:]] (for local tests)
    return records
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum([vr_a*vr_b for vr_a, vr_b in zip(voting_dict[sen_a], voting_dict[sen_b])])
...
def most_similar(sen, voting_dict):
    ...
    """
    return max([(s, policy_compare(s, sen, voting_dict))   \
                for s in voting_dict.keys() if s != sen ], \
                key=lambda x: x[1])[0]
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    return min([(s, policy_compare(s, sen, voting_dict))   \
                for s in voting_dict.keys() if s != sen ], \
                key=lambda x: x[1])[0]
...
#with open("voting_record_dump109.txt") as f:
#    democrats = create_voting_dict(f, 'D')
#avg_sims = [ (s,  find_average_similarity(s, democrats.keys() - {s}, democrats)) \
#             for s in democrats]
#print(max(avg_sims, key=lambda x: x[1])[0])


from functools import reduce

def vec_add(v1, v2): return [e1 + e2 for e1, e2 in zip(v1, v2)]
def vec_mul_scal(v, s): return [s*e for e in v]
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return vec_mul_scal(reduce(vec_add,[voting_dict[s] for s in sen_set]), 1.0 / len(sen_set))

#with open("voting_record_dump109.txt") as f:
#    democrats = create_voting_dict(f, 'D')
#print(find_average_record(democrats.keys(), democrats))

average_Democrat_record = [-0.16279069767441862, -0.23255813953488372, 1.0, 0.8372093023255813, 0.9767441860465116, -0.13953488372093023, -0.9534883720930233, 0.813953488372093, 0.9767441860465116, 0.9767441860465116, 0.9069767441860465, 0.7674418604651163, 0.6744186046511628, 0.9767441860465116, -0.5116279069767442, 0.9302325581395349, 0.9534883720930233, 0.9767441860465116, -0.3953488372093023, 0.9767441860465116, 1.0, 1.0, 1.0, 0.9534883720930233, -0.4883720930232558, 1.0, -0.32558139534883723, -0.06976744186046512, 0.9767441860465116, 0.8604651162790697, 0.9767441860465116, 0.9767441860465116, 1.0, 1.0, 0.9767441860465116, -0.3488372093023256, 0.9767441860465116, -0.4883720930232558, 0.23255813953488372, 0.8837209302325582, 0.4418604651162791, 0.9069767441860465, -0.9069767441860465, 1.0, 0.9069767441860465, -0.3023255813953488]

#d_avg_sim = [(s, sum([e1*e2 for e1, e2 in zip(v, average_Democrat_record)])) for s, v in democrats.items()]
#print(max(d_avg_sim, key=lambda x: x[1])[0])
...
def bitter_rivals(voting_dict):
    ...
    sim = float("inf")
    sens = (None, None)
    for s1 in voting_dict.keys():
        for s2 in voting_dict.keys():
            if s2 <= s1: continue
            new_sim = policy_compare(s1, s2, voting_dict)
            if sim <= new_sim: continue
            sim = new_sim
            sens = (s1, s2)
    return sens
