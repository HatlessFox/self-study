...
def signum(u):
    ...
    Example:
        ...
        True
    '''
    return Vec(u.D, {d:1 if 0 <= u[d] else -1 for d in u.D})
...
def fraction_wrong(A, b, w):
    ...
    Example:
        ...
        0.33333333333333337
    '''
    return .5 - signum(A*w)*b / len(b.D) / 2
...
def gradient_descent(A, b, w, sigma, T):
    ...
    for i in range(T):
        ...
        if i % 100 != 0: continue;
        print("#{:5} -- Loss: {:8.2}; Wrong: {:.1%}".format(
            i, loss(A, b, w), fraction_wrong(A, b, w)))
    return w        

if DO_UNGRADED_TESTS:
    ## Task 8.4.1, 8.6.11. 8.4.12
    A, b = read_training_data("train.data")
    init = .5
    w = Vec(A.D[1], {d:init for d in A.D[1]})
    w = gradient_descent(A, b, w, 10e-10, 6000)

    ## 8.4.13
    A_act, b_act = read_training_data("validate.data")
    print("== Validate ==")
    print("Wrong: {:.1%}".format(fraction_wrong(A_act, b_act, w)))
