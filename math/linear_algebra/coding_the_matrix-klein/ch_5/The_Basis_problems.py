...
lin_dep_R_1 = [-1, 0.5, 0.5]
lin_dep_R_2 = [-4, 1, -4/7]
lin_dep_R_3 = [-0.1, 0, 0, 1/3, 1]
...
def is_superfluous(L, i):
    ...
    if len(L) == 1:
        return len(L[0].f) == 0
    basis = coldict2mat(L[:i] + L[i+1:])
    residual = basis * solve(basis, L[i]) - L[i]
    return fabs(residual*residual) < 1e-20
...
def subset_basis(T):
    ...
    S = []
    for v in T:
        if not is_independent([v] + S): continue
        S += [v]
    return S
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    coords = solve(coldict2mat(S), z)
    for i, c in coords.f.items():
        if c == 0 or S[i] in A: continue
        return S[i]
    raise Error("Exchange Lemma failed ?!")
